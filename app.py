...
def simpan_ke_database(
    jenis_laporan, nama_file, file_bytes, spbu_id, total_baris
):
...
st.markdown(
    "**SPBU Monitoring System | JBT & JBKP Advanced Fraud Detection & Evidence**"
)
st.markdown("---")
...
st.sidebar.markdown("---")
st.sidebar.info(
    "💡 **Tips:** Setiap file yang Anda unduh akan otomatis disimpan ke dalam **Bank Database Arsip** lokal dan dapat diakses kembali kapan saja."
)
...
with tab_upload:
    ...
    st.markdown(
        "<p style='color: #64748b; font-size: 0.85rem; margin-top: -10px; margin-bottom: 20px;'>Silakan unggah file transaksi dalam format Excel (.xlsx) atau CSV, lalu sesuaikan pemetaan kolom tabel agar sistem dapat membaca data dengan benar.</p>",
        unsafe_allow_html=True,
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(
                f"File **{uploaded_file.name}** berhasil dimuat! Total baris data: {len(df_raw):,}"
            )
            ...
            default_vol = find_best_column(
                ["volume", "liter", "vol", "qty", "jumlah"]
            )
            ...
            with map_c1:
                col_nopol_opt = st.selectbox(
                    "Kolom Plat Nomor / Nopol",
                    columns_list,
                    index=columns_list.index(default_nopol)
                    if default_nopol in columns_list
                    else 0,
                )
                col_vol_opt = st.selectbox(
                    "Kolom Volume (L)",
                    columns_list,
                    index=columns_list.index(default_vol)
                    if default_vol in columns_list
                    else 0,
                )
            with map_c2:
                col_produk_opt = st.selectbox(
                    "Kolom Produk / Jenis BBM",
                    columns_list,
                    index=columns_list.index(default_produk)
                    if default_produk in columns_list
                    else 0,
                )
                col_time_opt = st.selectbox(
                    "Kolom Waktu / Jam Transaksi",
                    columns_list,
                    index=columns_list.index(default_time)
                    if default_time in columns_list
                    else 0,
                )
            with map_c3:
                col_nozzle_opt = st.selectbox(
                    "Kolom Nozzle / Pompa (Opsional)",
                    columns_list,
                    index=columns_list.index(default_nozzle)
        ...
        st.info(
            "👆 Silakan unggah file Excel atau CSV di atas untuk memulai analisis."
        )
...
if uploaded_file is not None and df_raw is not None:
    try:
        ...
        def deteksi_kategori_dan_kuota(plat_str, produk_str):
            ...
            is_jbt = any(
                x in prod_upper
                for x in ["SOLAR", "BIOSOLAR", "JBT", "MHD", "DEALITE"]
            )
            ...
            if is_jbt:
                if 1 <= nomor_reg <= 2999:
                    return (
                        "Mobil Pribadi (R4)",
                        st.session_state.kuota_pribadi_r4,
                    )
                    ...
                    return (
                        "Truk & Kendaraan Beban Berat",
                        st.session_state.kuota_berat,
                    )
                else:
                    return (
                        "Mobil Pribadi (R4)",
        ...
        total_vol = (
            pd.to_numeric(df_display[col_vol_opt], errors="coerce")
            .fillna(0)
            .sum()
        ...
        if col_time_opt in df_analysis.columns:
            ...
            df_analysis["is_fast_interval"] = (
                df_analysis["diff_minutes"]
                <= st.session_state.min_jeda_waktu
            )
        else:
        ...
        if (
            col_nozzle_opt in df_analysis.columns
            and col_time_opt in df_analysis.columns
        ):
        ...
        with tab1:
            ...
            st.markdown(
                "<p style='color: #64748b; font-size: 0.85rem; margin-top: -10px; margin-bottom: 20px;'>Total pengisian plat sama dalam 1 hari vs batas. Diurutkan: yang lewat kuota di atas. Perkiraan jenis = lead, wajib dicek CCTV/SAMSAT.</p>",
                unsafe_allow_html=True,
            )
            ...
            if not df_display.empty and col_nopol_opt in df_display.columns:
                ...
                mobil_helikopter_count = len(
                    df_g_metric[
                        df_g_metric["freq"]
                        > st.session_state.max_frekuensi_harian
                    ]
                )
                ...
                single_cap_violations = len(
                    df_analysis[
                        vol_numeric > st.session_state.batas_sekali_isi
                    ]
                )
            ...
            def render_custom_metric(
                label, value, icon, alert_if_gt_zero=False
            ):
            ...
            with c1:
                render_custom_metric(
                    ...
                    f"{total_vol:,.1f} L",
                    "📈",
                    alert_if_gt_zero=False,
                )
            with c2:
                render_custom_metric(
                    "Total Transaksi",
                    f"{total_transaksi:,} Baris",
                    "📋",
                    alert_if_gt_zero=False,
                )
            with c3:
            ...
            with c4:
                render_custom_metric(
                    "SPBU ID", spbu_id_input, "🏢", alert_if_gt_zero=False
                )
            ...
            with f_col1:
                if st.button(
                    f"⛽ JBT · Solar ({jbt_count:,})", use_container_width=True
                ):
                    st.session_state.filter_produk = "JBT"
                    st.rerun()
            with f_col2:
                if st.button(
                    f"⛽ JBKP · Pertalite ({jbkp_count:,})",
            ...
            if not df_analysis.empty and col_nopol_opt in df_analysis.columns:
                agg_dict = {
                    ...
                    "sample_produk": (col_produk_opt, "first")
                    if col_produk_opt in df_analysis.columns
                    else (col_vol_opt, lambda x: "JBT"),
                    "has_fast_interval": ("is_fast_interval", "any"),
                    "has_cross_pump": ("is_cross_pump", "any"),
                }

                df_grouped = df_analysis.groupby(col_nopol_opt).agg(
                    **agg_dict
                ).reset_index()
                ...
                for index, row in df_grouped.iterrows():
                    ...
                    prod_val = (
                        row["sample_produk"]
                        if "sample_produk" in row
                        else "JBT"
                    )
                    is_fast = row["has_fast_interval"]
                    is_cp = row["has_cross_pump"]

                    jenis_kendaraan, target_kuota = (
                        deteksi_kategori_dan_kuota(plat, prod_val)
                    )
                    is_helikopter = (
                        freq > st.session_state.max_frekuensi_harian
                    )
                    persen = (
                        int((vol / target_kuota) * 100)
                        if target_kuota > 0
                        else 100
                    )
        ...
        with tab2:
            ...
            with control_col3:
                ...
                with pd.ExcelWriter(
                    output_tindak_lanjut, engine="openpyxl"
                ) as writer:
                    df_analysis.to_excel(
                        writer, index=False, sheet_name="Tindak_Lanjut"
                    )
                ...
                if st.button(
                    "Simpan & Unduh Tindak Lanjut (Excel)",
                    use_container_width=True,
                    key="btn_dl_tl",
                ):
                    simpan_ke_database(
                    ...
                    st.success(
                        "✅ File berhasil disimpan ke Bank Database & diunduh!"
                    )
            ...
            with control_col4:
                ...
                df_export["Status_Evidens_Foto"] = [
                    "ADA FOTO"
                    if f"row_{idx}" in st.session_state.foto_evidens
                    else "BELUM ADA FOTO"
                ...
                with pd.ExcelWriter(
                    output_transaksi, engine="openpyxl"
                ) as writer:
                    df_export.to_excel(
                        writer, index=True, sheet_name="Transaksi_Dan_Foto"
                    )
                ...
                st.download_button(
                    ...
                    on_click=simpan_ke_database,
                    args=(
                        "Transaksi Lengkap & Foto Evidens",
                        filename_full,
                        final_excel_data,
                        spbu_id_input,
                        len(df_export),
            ...
            if search_query.strip():
                df_filtered_detail = df_filtered_detail[
                    df_filtered_detail[col_nopol_opt]
                    .astype(str)
            ...
            if not df_filtered_detail.empty:
                for idx, row in df_filtered_detail.iterrows():
                    ...
                    waktu_val = (
                        ...
                        if col_time_opt in df_filtered_detail.columns
                        else "31/08/2026, 05.45.36"
                    )
                    produk_val = (
                        str(row[col_produk_opt])
                        if col_produk_opt in df_filtered_detail.columns
                        else "BIO_SOLAR"
                    )

                    nozzle_code = (
                        str(row[col_nozzle_opt])
                        if col_nozzle_opt in df_filtered_detail.columns
                        and pd.notna(row[col_nozzle_opt])
                        else "H1"
                    )
                    ...
                    plat_val_display = (
                        "– tanpa plat –"
                        if plat_raw_val in ["INVALID_NOPOL", ""]
                        else plat_raw_val
                    )

                    vol_numeric_val = (
                        pd.to_numeric(row[col_vol_opt], errors="coerce")
                        if col_vol_opt in df_filtered_detail.columns
                        else 0.0
                    )
                    vol_val = (
                        f"{vol_numeric_val:.2f}L"
                        if pd.notna(vol_numeric_val)
                        else "0.00L"
                    )

                    perkiraan_jenis, _ = deteksi_kategori_dan_kuota(
                        plat_raw_val, produk_val
                    )
                    ...
                    if plat_raw_val in ["– tanpa plat –", "INVALID_NOPOL", ""]:
                        ...
                        row.get("is_fast_interval", False)
                        or row.get("is_cross_pump", False)
                        or vol_numeric_val > st.session_state.batas_sekali_isi
                    ):
                        alasan = (
                            f"Total harian melewati kuota / jeda singkat (<30m)"
                        )
                        status_badge_html = "<span style='background-color: #fef3c7; color: #92400e; padding: 4px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 600;'>● Perlu Diperiksa</span>"
